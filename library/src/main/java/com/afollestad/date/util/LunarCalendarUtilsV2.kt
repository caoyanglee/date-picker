...
object LunarCalendarUtilsV2 {
    ...
    fun getLunarString(year: Int, month: Int, day: Int): String {
        ...
        val Festival = getFestival(year, month, day)
    ...
    fun getFestival(year: Int, month: Int, day: Int): String { //获取公历对应的节假日或二十四节气
        //农历节假日
        val `var` = getLunarDateINT(year, month, day)
        val lun_year = `var` / 10000
        val lun_month = (`var` % 10000) / 100
        val lun_Day = (`var` - lun_year * 10000 - lun_month * 100)
        for (i in lunarHoliday.indices) { // 返回农历节假日名称
            val ld =
                lunarHoliday[i].split(" ").toTypedArray()[0] // 节假日的日期
            val ldv =
                lunarHoliday[i].split(" ").toTypedArray()[1] // 节假日的名称
        ...
        for (i in solarHoliday.indices) {
            ...
            // 返回公历节假日名称
            val sd =
                solarHoliday[i].split(" ").toTypedArray()[0] // 节假日的日期
            val sdv =
                solarHoliday[i].split(" ").toTypedArray()[1] // 节假日的名称
    ...
    fun getLunarDate(
        ...
        //农历月份
        month = (`var` % 10000) / 100
        lunarMonth = chineseNumber[(month - 1) % 12] + "月" // 设置对应的阴历月份
        ...
        return if (day == 1) chineseNumber[(month - 1) % 12] + "月" else getChinaDayString(
            day
        )
    }

    override fun toString(): String {
        return if (chineseNumber[(month - 1) % 12] === "正" && getChinaDayString(
                day
            ) === "初一"
        ) "农历" + year + "年" else if (getChinaDayString(day) === "初一") chineseNumber[(month - 1) % 12] + "月" else getChinaDayString(
            day
        )
    ...
    fun getConstellation(month: Int, day: Int): String { //计算星座
        ...
        var constellation = ""
        constellation = if (date >= 120 && date <= 218) {
            constellations[1]
    ...
    fun jcrt(d: String): String {
        ...
        //String yj0 = "",yj1 = "",br = "-";
        if (d === "建") jcrjxt =
            yj0 + yi_string[0] + br + yj1 + ji_string[0]
        if (d === "除") jcrjxt =
            yj0 + yi_string[1] + br + yj1 + ji_string[1]
        if (d === "满") jcrjxt =
            yj0 + yi_string[2] + br + yj1 + ji_string[2]
        if (d === "平") jcrjxt =
            yj0 + yi_string[3] + br + yj1 + ji_string[3]
        if (d === "定") jcrjxt =
            yj0 + yi_string[4] + br + yj1 + ji_string[4]
        if (d === "执") jcrjxt =
            yj0 + yi_string[5] + br + yj1 + ji_string[5]
        if (d === "破") jcrjxt =
            yj0 + yi_string[6] + br + yj1 + ji_string[6]
        if (d === "危") jcrjxt =
            yj0 + yi_string[7] + br + yj1 + ji_string[7]
        if (d === "成") jcrjxt =
            yj0 + yi_string[8] + br + yj1 + ji_string[8]
        if (d === "收") jcrjxt =
            yj0 + yi_string[9] + br + yj1 + ji_string[9]
        if (d === "开") jcrjxt =
            yj0 + yi_string[10] + br + yj1 + ji_string[10]
        if (d === "闭") jcrjxt =
            yj0 + yi_string[11] + br + yj1 + ji_string[11]
        return jcrjxt
    }
    ...
    fun getyiji(year: Int, month: Int): ArrayList<String?> {
        ...
        for (i in 0 until days_of_month) {
            if (mLDay > mLun_x) { //Log.i("","mLDay > mLun_x "+mLDay+":"+mLun_x);
                val `var` = getLunarDateINT(year, month, i + 1)
                val mLYear = `var` / 10000
                mLMonth = (`var` % 10000) / 100
                mLDay = (`var` - mLYear * 10000 - mLMonth * 100)
                mLun_x =
                    if (leapMonth(mLYear) != 0) leapDays(
                        mLYear
                    ) else monthDays(mLYear, mLMonth)
                //Log.i("","mLDay > mLun_x ?"+mLDay+":"+mLun_x);
            }
            //依节气调整二月分的年柱, 以立春为界
            if (month == 2 && i + 1 == term2) { //cY = cyclicalm(year-1900 + 36);
                num_y = year - 1900 + 36
            }
            //依节气 调整月柱, 以「节」为界
            if (i + 1 == firstNode) {
...
//立春日期
        val term2 = sTerm(year, 2)
        val firstNode = sTerm(year, (month - 1) * 2) //当月的24节气中的节开始日
        if (month == 2 && day == term2) { //cY = cyclicalm(year-1900 + 36);//依节气调整二月分的年柱, 以立春为界
            num_y = year - 1900 + 36
        }
        if (day == firstNode) {
    ...
    fun getChinaDayString(day: Int): String { //将农历day日格式化成农历表示的字符串
